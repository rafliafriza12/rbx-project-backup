...
function generateInvoiceMessage(transactions: any[]): string {
  ...
  if (isMultipleItems) {
    ...
    lines.push(
      `**Kode Pesanan:** \`${mainTransaction.midtransOrderId || mainTransaction.invoiceId}\``,
    );
  } else {
  ...
  if (mainTransaction.paidAt) {
    lines.push(
      `**Waktu Pembayaran:** ${formatDate(new Date(mainTransaction.paidAt))}`,
    );
  }

  lines.push("");
  lines.push(
    `### 📦 Detail Pesanan${isMultipleItems ? ` (${transactions.length} item)` : ""}`,
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "Silakan hubungi kami di chat ini jika ada kendala atau pertanyaan seputar pesanan Anda.",
  );
  lines.push("");
    ...
    if (isAdmin) {
      ...
      const formattedRooms = chatRooms.map((room: any) => ({
        ...
        userId: {
          ...
          fullName:
            `${room.userId.firstName || ""} ${room.userId.lastName || ""}`.trim() ||
      ...
      console.log(
        `[GET /rooms] ✅ User: Returned ${formattedRooms.length} rooms`,
      );
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === "order") {
      if (!transactionCode) {
        return NextResponse.json(
          { error: "Transaction code required for order support" },
          { status: 400 },
        );
      }
    ...
    try {
      ...
      console.error(
        "[POST /rooms] ⚠️ Pusher error for new room notification:",
        pusherError,
      );
    }
        ...
        if (mainTransaction) {
          ...
          if (mainTransaction.midtransOrderId) {
            ...
            if (relatedTransactions.length > 1) {
              ...
              console.log(
                `[POST /rooms] 📦 Found ${relatedTransactions.length} items in this order`,
              );
            }
          }
          ...
          try {
            ...
            const messagePayload = {
              ...
              senderId: {
                ...
                fullName:
                  `${(populatedMessage as any).senderId.firstName || ""} ${(populatedMessage as any).senderId.lastName || ""}`.trim() ||
                  (populatedMessage as any).senderId.username,
                avatar: (populatedMessage as any).senderId.profilePicture,
              },
            };

            await pusher.trigger(
              `private-chat-room-${chatRoom._id}`,
              "new-message",
              {
            ...
            await pusher.trigger("admin-notifications", "new-message", {
              ...
              senderName:
                (populatedMessage as any).senderId?.fullName ||
                (populatedMessage as any).senderId?.username,
              roomType: type,
            ...
            console.log(
              "[POST /rooms] 📨 Auto invoice message sent via Pusher",
            );
          } catch (pusherError) {
            console.error(
              "[POST /rooms] ⚠️ Pusher error (non-critical):",
              pusherError,
            );
          }

          console.log(
            "[POST /rooms] 📝 Auto invoice message created for order chat",
          );
        } else {
          console.log(
            "[POST /rooms] ⚠️ Transaction not found for code:",
            transactionCode,
          );
        }
      } catch (autoMsgError) {
        console.error(
          "[POST /rooms] ⚠️ Failed to create auto message:",
          autoMsgError,
        );
    ...
    const response = {
      ...
      userId: {
        ...
        fullName:
          `${chatRoom.userId.firstName || ""} ${chatRoom.userId.lastName || ""}`.trim() ||
      ...
      adminId: {
        ...
        fullName:
          `${chatRoom.adminId.firstName || ""} ${chatRoom.adminId.lastName || ""}`.trim(),
      },
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const userIds = [
      ...new Set(
        allRooms.map((room) => room.userId?._id?.toString()).filter(Boolean),
      ),
    ];
    ...
    console.log(
      `[Bulk Delete] 🗑️ Deleted ${deletedMessages.deletedCount} messages`,
    );

    // Delete all chat rooms
    const deletedRooms = await ChatRoom.deleteMany({});
    console.log(
      `[Bulk Delete] 🗑️ Deleted ${deletedRooms.deletedCount} chat rooms`,
    );
