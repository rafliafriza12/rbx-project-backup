...
export default function Rbx5Page() {
  ...
  // Gamepass amount dari server (mempertimbangkan Robux Plus)
  const [serverGamepassAmount, setServerGamepassAmount] = useState<number>(0);
  const [isRobuxPlusAvailable, setIsRobuxPlusAvailable] = useState(false);
  const [isFetchingGamepassAmount, setIsFetchingGamepassAmount] = useState(false);
  ...
  const calculatePriceFromRobux = (robuxAmount: number) => {
    if (robuxAmount <= 0) return 0;

    // Use currentRobuxPricing if available, otherwise fall back to stats.hargaPer100Robux
    const rate = currentRobuxPricing?.pricePerHundred || stats.hargaPer100Robux || 13000;

    // Calculate based on price per 100 robux
    return Math.ceil((robuxAmount / 100) * rate);
  };

  // Get current price for display (use dynamic pricing if available)
  const getCurrentPrice = () => {
    if (robux > 0) {
      if (currentRobuxPricing || !selectedPackage) {
    ...
    // Fetch tidak diperlukan lagi karena harga gamepass selalu base (× 1.43)
    // Robux Plus discount ditangani otomatis oleh Roblox saat purchase, bukan di frontend
    useEffect(() => {
      // Reset serverGamepassAmount saat robux berubah (untuk consistency)
      setServerGamepassAmount(0);
      setIsRobuxPlusAvailable(false);
    }, [robux]);


    // Hitung gamepass amount dari lokal (selalu base × 1.43)
    // Robux Plus discount ditangani Roblox secara otomatis saat purchase
    ...
    // isGamepassAmountReady selalu true karena kalkulasi lokal (tidak perlu fetch)
    const isGamepassAmountReady = robux > 0;

    // Function to check if gamepass exists
    // silent=true: auto-check on window focus, suppress error toasts to avoid spam
    const checkGamepassExists = async (silent = false) => {
      ...
      try {
        // Hanya tunggu jika SEDANG fetching (bukan jika belum fetch)
        if (isFetchingGamepassAmount) {
          if (!silent) toast.warning("Sedang menyiapkan harga gamepass, tunggu sebentar lalu coba lagi.");
        ...
        if (expectedRobux <= 0) {
          if (!silent) toast.error("Jumlah robux tidak valid.");
          setIsCheckingGamepass(false);
          return;
        }
        ...
        if (!universeId) {
          if (!silent) toast.error("Universe ID tidak ditemukan. Mohon pilih game terlebih dahulu.");
          setIsCheckingGamepass(false);
          return;
        }
        ...
        if (data.success) {
          ...
          // Only show error on manual check, not auto-check
    ...
    // Auto-check gamepass when user returns to the tab (window focus)
    // This handles the common case: user opens Roblox → creates gamepass → switches back
    useEffect(() => {
      const handleWindowFocus = () => {
        if (
          ...
          robux > 0 &&
          isGamepassAmountReady   // Pastikan harga sudah siap dari server
        ) {
          checkGamepassExists(true);
        }
      };
    ...
    }, [currentStep, selectedPlace, isCheckingGamepass, gamepassInstructionShown, robux, isGamepassAmountReady]);
    ...
    const isStep2Valid =
      selectedPlace !== null &&
      gamepassInstructionShown &&
      lastCheckedRobuxAmount === robux; // Gamepass must be verified with the current robux amount
