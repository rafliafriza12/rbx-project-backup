...
export default function UserChatPage() {
  ...
  useEffect(() => {
    ...
    // Note: System message is handled by private room channel in UserChatInterface component
    userChannel.bind("room-deactivated", (data: { roomId: string }) => {
      setChatRooms((prev) =>
        prev.map((room) =>
          room._id === data.roomId
          ...
          <div className="bg-gradient-to-r from-primary-800/95 to-primary-900/95 rounded-xl border border-white/20 overflow-hidden backdrop-blur-sm shadow-xl">
            ...
            >
                ...
                {selectedRoom ? (
                  <>
                    <div className="relative flex-shrink-0">
                      ...
                      {/* Status indicator */}
                      <div
                        className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-primary-800 ${
                          selectedRoom.status === "active"
                            ? "bg-emerald-fresh"
                            : "bg-gray-500"
                        }`}
                  ...
                  <>
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-white/60 font-semibold text-sm">
                        Pilih Chat Room
                      </h3>
                      <p className="text-white/40 text-xs">
                        {chatRooms.length} percakapan tersedia
                      </p>
                    </div>
                  </>
                )}
              ...
              <div className="flex items-center gap-2">
                {/* Total unread badge */}
                {(chatRooms.some((r) => r.unreadCountUser > 0) ||
                  newMessageRooms.size > 0) && (
                  <div className="w-6 h-6 bg-red-500 rounded-full flex items-center justify-center">
                    <span className="text-white text-xs font-bold">
                      {chatRooms.reduce(
                        (sum, r) => sum + r.unreadCountUser,
                        0,
                      ) + newMessageRooms.size}
                    </span>
                  </div>
                )}
                ...
                <div
                  className={`p-1.5 rounded-lg bg-white/10 transition-transform duration-300 ${showMobileDropdown ? "rotate-180" : ""}`}
                >
                  <svg
                    className="w-5 h-5 text-white"
                    fill="none"
                      ...
                      {chatRooms.map((room) => (
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            ...
                            {selectedRoomId === room._id && (
                              <svg
                                className="w-4 h-4 text-neon-pink flex-shrink-0"
                                fill="currentColor"
                                viewBox="0 0 24 24"
                              >
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className="relative flex-shrink-0">
                          ...
                          {/* Status indicator */}
                          <div
                            className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-primary-800 ${
                              room.status === "active"
                                ? "bg-emerald-fresh"
                                : "bg-gray-500"
                            }`}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            <h3 className="text-white font-semibold text-sm truncate flex items-center gap-1.5">
                              {room.roomType === "general" ? (
                                ...
                                <>
                                  Chat for Order{" "}
                                  {room.transactionCode?.slice(-8) || "Support"}
                                </>
                              )}
                            </h3>
                            {room.lastMessageAt && (
                          ...
                          <p className="text-white/60 text-xs truncate">
                            {room.lastMessage ||
                              (room.roomType === "general"
                                ? "Chat umum dengan admin"
                                : `Order: ${room.transactionCode || "N/A"}`)}
                          </p>
                        </div>
                      </div>
                    </button>
              ...
              <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-primary-800 via-primary-700 to-primary-900">
                <div className="text-center max-w-md px-6">
                  <div className="w-20 h-20 md:w-24 md:h-24 mx-auto mb-4 md:mb-6 bg-gradient-to-br from-neon-purple/20 to-neon-pink/20 rounded-full flex items-center justify-center border border-white/10">
                    <svg
                      className="w-10 h-10 md:w-12 md:h-12 text-neon-purple"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  <p className="text-white/60 text-xs md:text-sm mb-4">
                    <span className="md:hidden">
                      Tap dropdown di atas untuk memilih chat atau buat chat
                      baru
                    </span>
                    <span className="hidden md:inline">
                      Pilih ruang chat dari sidebar atau buat ruang chat baru
                    </span>
                  </p>
                  <button
                    onClick={() => {
